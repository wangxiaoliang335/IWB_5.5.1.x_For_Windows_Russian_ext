...
//YUY2ToRGB
//©°  ©´ ©°                   ©´©°       ©´
//©¦B ©¦ ©¦1.164  2.018  0.0  ©¦©¦V - 128©¦
//©¦G ©¦=©¦1.164 -0.391 -0.813©¦©¦U - 128©¦
//©¦R ©¦ ©¦1.164  0.0    1.596©¦©¦Y - 16 ©¦
//©¸  ©¼ ©¸                   ©¼©¸       ©¼
//


//@function: convert yuy2 to rgb
//@paramteres: yuy2, pointer which points to yuy2 source data, is input parameter
//             widht, the width of image
//             height, the height of image
//             rgb, pointer which points to rgb destination buffer, is output parameter.
inline _declspec(naked) void YUY2ToRGB_MMX(
    const unsigned char* yuy2,
    unsigned int size,
    unsigned char* rgb)
{
#define RGBY(i) {\
...
#define RGBV(i) {\
    ...
    _asm
    {
        ...
        mov esi, [esp + 32 +  4];//pointer to yuy2 data
        mov ecx, [esp + 32 +  8];//pixel count;
        mov edi, [esp + 32 + 12];//pointer to rgb data
...
_next_4_pixels:
        ...
        //al=Y0
        mov al, bl
        movq mm0, [CoefficientsRGBY + 8*eax]

        //al=U0
        shr ebx, 8
        mov al, bl;
        movq mm2, [CoefficientsRGBU + 8*eax]
        ...
        shr ebx, 8
        mov al, bl;
        movq mm3, [CoefficientsRGBV + 8*eax]
       ...
       //mm1=00 B1 G1 R1
        paddw mm1, mm2
        paddw mm1, mm3
        //mm1=00 B1/64 G1/64 R1/64
        psraw mm1, 6



        //packuswb a, b
        //Pack the four 16-bit values from a into the lower four 8-bit values of the result with unsigned saturation,
        //and pack the four 16-bit values from b into the upper four 8-bit values of the result with unsigned saturation.
        //mm0=00 B1 G1 R1 B0 G0 R0 00 
        packuswb mm0, mm1;//

        //mm0 = 00 00 B1 G1 R1 B0 G0 R0
        psrlq mm0, 8;
       
        
        //movd eax, mm0

        ////mm1=00 00 00 00 00 B0 G0 R0
        //movd mm1, eax;

        //xor eax,eax
        ////mm0=00 B1 G1 R1 00 B0 00 00
        //pinsrw mm0, eax,0

        ////mm0=00 B1 G1 R1 00 00 00 00
        //pinsrw mm0, eax,1

        ////mm0= 00 00 B1 G1 R1 00 00 00
        //psrlq mm0, 8
        ...
        shr ebx, 8
        mov al, bl;
        movq mm4, [CoefficientsRGBV + 8*eax]

        //mm1 = 0 B2 G2 R2
        paddw mm1, mm2 
        paddw mm1, mm4

        //mm1 = 0 B2/64 G2/64 R2/64
        psraw mm1, 6
        //mm1 = B2 G2 R2 00
        psllq mm1, 16;
        ...
        //mm3 = 0 B3/64 G3/64 R3/64
        psraw mm3, 6

        //mm1=00 B3 G3 R3 B2 G2 R2 00 
        packuswb mm1, mm3;//

        //mm1=00 00 B3 G3 R3 B2 G2 R2 
        psrlq mm1, 8;


        //mm3=00 00 B3 G3 R3 B2 G2 R2 
        movq mm3, mm1;

        //mm3=G2 R2 00 00 00 00 00 00
        psllq mm3, 48

       //mm0 = 00 00 B1 G1 R1 B0 G0 R0
       //mm3 = G2 R2 00 00 00 00 00 00
       //mm0 = G2 R2 B1 G1 R1 B0 G0 R0 
       por mm0, mm3;
       ...
       add edi, 8;//
        ...
        //mm1 =  00 00 00 00 B3 G3 R3 B2 
        psrlq mm1, 16;
        movd [edi], mm1;
        add edi,4

        sub  ecx, 4;//4 pixel each time

        jnz _next_4_pixels;//continue to net
...
//@Comments: Width must be even number.
inline void YUY2ToRGB(
    const unsigned char* yuy2,
    int width,
    int height,
    unsigned char* rgb)
{
    YUY2ToRGB_MMX(yuy2, width*height, rgb);
}



////===========
////@function: convert yuy2 to DIB(device independent bitmap) in which bitmap is stored upside down.
////@paramteres: yuy2, pointer which points to yuy2 data, is input parameter
////             widht, the width of image
////             height, the height of image
////             rgb, pointer which points to rgb destination buffer, is output parameter.
//
inline _declspec(naked) void YUY2ToDIB_MMX(
    const unsigned char* pYUY2,
    unsigned short nWidth,
    unsigned short nHeight,
    unsigned char* pFirstLineData)
{
#define RGBY(i) {\
    (short)(1.164 * 64 * (i - 16) + 0.5), \
    (short)(1.164 * 64 * (i - 16) + 0.5), \
...
#define RGBV(i) {\
    ...
    _asm
    {
        ...
        mov edi, [esp + 32 + 16 ];//pointer to rgb data

        xor eax, eax
        mov eax, ecx;
       imul eax,eax,6;//
       mov  ebp, eax ;//ebp = width*6
...
inline void YUY2ToDIB(
    const unsigned char* yuy2,
    int width,
    int height,
    unsigned char* rgb)
{
    unsigned char*  pFirstRGBLineData = rgb + (height-1)*width*3;
    
    YUY2ToDIB(yuy2, width, height, pFirstRGBLineData);
}