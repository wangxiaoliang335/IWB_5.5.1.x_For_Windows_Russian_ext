...
void CDib::Draw(HDC hDC,int nWidth, int nHeight, int nDestLeft , int nDestTop)
{
	if(m_pDib!=NULL)
	{
		//ASSERT(IsValid());
...
BOOL CDib::Save(const TCHAR * pzFileName)
{
	...
	bmpFileHeader.bfSize = pBmpInfoHeader->biSizeImage + sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER);
...
//BOOL CDib::SaveDisk(const TCHAR *pzFileName)
//{
//	std::ofstream file;
//	file.open(pzFileName,std::ios_base::binary | std::ios_base::out);
//
//	if(file.fail())
//		goto exit;
//
...
void CDib::Rectangle(const RECT& rc, COLORREF clr)
{
    long x = rc.left;
    long y = rc.top;

    for(; x < rc.right;  x++) SetPixel(x, y, clr);
    for(; y < rc.bottom; y++) SetPixel(x, y, clr);
    for(; x > rc.left;   x--) SetPixel(x, y, clr);
    for(; y > rc.top ;   y--) SetPixel(x, y, clr);
