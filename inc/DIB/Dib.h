...
public:
    ...
    operator BITMAPINFOHEADER*(){return (BITMAPINFOHEADER*)m_pDib;}
    ...
    void Rectangle(const RECT& rc,COLORREF color);
//operations
public:
	BOOL Open(const TCHAR *pzFileName);
	BOOL Save(const TCHAR *pzFileName);
	//BOOL SaveDisk(const TCHAR *pzFileName);
	void Draw(HDC hDC,int nWidth, int nHeight, int nDestLeft = 0, int nDestTop = 0);
	void Close();
