...
namespace MatlabSimu
{
    ...
    {
    public:
        typedef ColIterator<T> col_iterator_type;
        typedef Matrix<T>      matrix_type;
    ...
    public:
        ...
        friend  T  Mean<T>(const Const_ColIterator& col);
    protected:
        const Matrix<T>* m_pMatrix;
        int              m_nColNo ;            
    };
    ...
    public:
        ...
        typedef Const_RowIterator<T> const_row_iterator;

        typedef ColIterator<T>             col_iterator;
        typedef Const_ColIterator<T> const_col_iterator;

        typedef  SubmatrixAccessor<T> submatrix_accessor;    
    
        friend class row_iterator;
        friend class const_row_iterator;
        friend class col_iterator;
        friend class const_col_iterator;
        friend class submatrix_accessor;

        friend Matrix  Mean<T>(const Matrix             & m  );
        friend  T      Mean<T>(const row_iterator       & row);
        friend  T      Mean<T>(const const_row_iterator & row);
        friend  T      Mean<T>(const col_iterator       & row);
        friend  T      Mean<T>(const const_col_iterator & row);

        friend  Matrix<T> Abs<T>(const Matrix<T>& m);
        friend  Matrix<T> Abs<T>(const row_iterator& row_it);
        friend  Matrix<T> Abs<T>(const col_iterator& col_it);

        friend  Matrix<T> Transpose<T>(const matrix_type & m);
        ...
        {
            SetDimension(m,n);
            memset(m_pData, 0, sizeof(T)*m_nRow*m_nCols);
        }
        ...
        {
            if(nRowNo < 0 || nRowNo >= m_nRows) throw std::exception("Invalid Row Number!");
       ...
       SubmatrixAccessor<T> operator()(const char* szRow, const char * szCol)
        ...
        {
            if(nColNo < 0 || nColNo >= m_nCols) throw std::exception("Invalid Column Number!");
    ...
    Matrix<T> Abs(const ColIterator<T>& col_it)
    {
        ...
        {
            abs_func<T> abs;         
            result.m_pData[r] = abs(row_it.m_pMatrix->m_pData[nOffset]);
            nOffset ++;
        }
    ...
    template<class T>  int SVD(const Matrix<T>& A, Matrix<T>* pS, Matrix<T>* pU , Matrix<T> *pV)
    {
        ...
        //                above for details.
         ...
         Matrix<T>  AA = A;
        char* JOBU  = (NULL == pU)?"N":"A";
        char* JOBVT = (NULL == pV)?"N":"A";
        ...
        int nLDA  = M      ;//The leading dimensionof the array A.
        int nLDU  = M      ;//The leading dimensionof the array U
        int nLDVT = N      ;//The leading dimensionof the array VT
        ...
        {
              printf( "%th argument has illegal value\n", abs(info));  
        }
