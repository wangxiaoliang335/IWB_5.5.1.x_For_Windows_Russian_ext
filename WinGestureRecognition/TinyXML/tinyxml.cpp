...
void TiXmlBase::PutString( const TIXML_STRING& str, TIXML_STRING* outString )
{
	...
	while( i<(int)str.length() )
	{
		...
		else if ( c < 32 )
		{
			...
			char buf[ 32 ];
			sprintf_s( buf, _countof(buf), "&#x%02X;", (unsigned) ( c & 0xff ) );
...
{	
	char buf[64];
	sprintf_s( buf, _countof(buf), "%d", val );
...
{	
	char buf[256];
	sprintf_s( buf, _countof(buf), "%0.2f", val );
	SetAttribute( name, buf );
}
...
bool TiXmlDocument::LoadFile( const char* filename, TiXmlEncoding encoding )
{
	...
	FILE* file = NULL;
    errno_t en = fopen_s(&file, value.c_str(), "r");
    if(0 != en) return false;
...
bool TiXmlDocument::SaveFile( const char * filename ) const
{
    ...
    FILE* fp = NULL;
    errno_t en= fopen_s(&fp, filename, "w");
	if (0 == en)
	{
		Print( fp, 0 );
		fclose( fp );
...
{
	char buf [64];
	sprintf_s (buf, _countof(buf), "%d", _value);
	SetValue (buf);
}
...
{
	char buf [256];
	sprintf_s (buf, _countof(buf), "%lf", _value);
	SetValue (buf);
}
