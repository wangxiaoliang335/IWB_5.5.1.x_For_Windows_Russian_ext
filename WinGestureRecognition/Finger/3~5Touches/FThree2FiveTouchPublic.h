...
//class CFThree2FiveTouchPublic
//{
//private:
//    CFThree2FiveTouchPublic(void);
//    ~CFThree2FiveTouchPublic(void);
//
//public:
...
//
//    /*
//    @功能：计算中心距，为了简单，把光斑集中每个点到中心点的距离的平方和当作中心距
//    @参数：aryLightSpots，光点数组
//           nCount，光点个数
//           ptMid，光斑集的中心点
//           nCentriMoments[in, out]，计算的中心距
//    @返回值：true,计算成功；false,计算失败
//    */
//    static bool CalcCentriMoments(const TLightSpot * aryLightSpots,  int nCount, const POINT &ptMid, int &nCentriMoments);
//
//public:
...
//
//    static POINT                m_ptCur[5];                 //当前采集坐标点
//    static POINT                m_ptSartMid;               //开始采集坐标点的中间值，实质上是本次移动（移动距离超过一定的阈值才认为移动）采集的开始
//    static POINT                m_ptLastMid;               //上一次采集坐标点的中间值
//    static POINT                m_ptLast[5];                //上一次采集坐标点
//    static int                  m_nLastPts;                 //上一次采集的坐标点数目
//
//    static int                  m_nLastCentriMoments;      //上一次采集光点的中心矩
//    static int                  m_curTimesCMIncreaced;     //中心矩增加的次数
//    static int                  m_curTimesCMDecreaced;      //中心矩减少的次数
//    static int                  m_nTotalCollects;           //采集总次数
//
