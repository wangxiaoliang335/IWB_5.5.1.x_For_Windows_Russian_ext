...
BOOL LoadConfig(TiXmlNode *pNode, GlobalSettings& globalSettings)
{
    ...
    do
    {
        ...
        if(_stricmp(lpszElementName, "Param") == 0)
        {
            ...
            else if (paramName && _stricmp(paramName, "CircleCalibrateXScale") == 0 && paramValue)
            {
                globalSettings.circleCalibrateXScale = atoi(paramValue);
            }
            else if (paramName && _stricmp(paramName, "CircleCalibrateYScale") == 0 && paramValue)
            {
                globalSettings.circleCalibrateYScale = atoi(paramValue);
            }
